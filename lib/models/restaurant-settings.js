...
class RestaurantSettings {
  ...
  constructor(row) {
    ...
    this.uncategorizedLabel = row.uncategorized_label ?? 'Uncategorized';
    this.primaryFont = row.primary_font ?? 'Inter, system-ui, sans-serif';
  ...
  static async get() {
    ...
    if (!rows[0]) {
      ...
      return new RestaurantSettings({
        id: 1,
        restaurant_name: 'Your Restaurant Here',
  ...
  static async update(settings) {
    console.log('🔍 UPDATE SETTINGS - Received:', JSON.stringify({
      uncategorizedLabel: settings.uncategorizedLabel,
    ...
    const { rows } = await pool.query(
      ...
      [
        ...
        settings.uncategorizedLabel ?? 'Uncategorized',
        settings.primaryFont ?? 'Inter, system-ui, sans-serif',
      ]
    );

    console.log('✅ UPDATE SETTINGS - Saved uncategorizedLabel:', rows[0]?.uncategorized_label);

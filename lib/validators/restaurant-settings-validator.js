...
const restaurantSettingsSchema = z.object({
  ...
  tagline: z.string()
    ...
    .optional()
    .nullable(),
  uncategorizedLabel: z.string()
    .max(50, 'Uncategorized label too long')
    .optional()
    .nullable()
    .transform((val) => val ? escapeHtml(val) : val),
  logoUrl: z.string()
    .refine(
      (val) => !val || val === '' || val.startsWith('data:image/') || /^https?:\/\/.+/.test(val),
  ...
  backgroundImageUrl: z.string()
    .refine(
      (val) => !val || val === '' || val.startsWith('data:image/') || /^https?:\/\/.+/.test(val),
      'Background image must be a valid URL or image file'
    )
    .optional()
    .nullable()
    .or(z.literal('')), // Allow empty string
